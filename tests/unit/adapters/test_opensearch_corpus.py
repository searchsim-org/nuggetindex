"""Unit tests for :class:`nuggetindex.adapters.opensearch.OpenSearchCorpus`.

The adapter duck-types on the client's ``.search(index=..., body=..., size=...)``
method -- no real ``opensearch-py`` package is required. Tests use
...
from nuggetindex.adapters import OpenSearchCorpus
...
async def test_search_translates_hits_to_documents():
    ...
    corpus = OpenSearchCorpus(client=mock_client, index="docs")
