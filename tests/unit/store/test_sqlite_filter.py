...
async def test_filter_active_at_query_time(populated):
    ids = await populated.afilter(
        query_time=datetime(2020, 1, 1, tzinfo=UTC), view="active"
    )
...
async def test_filter_all_ignores_status(populated):
    ids = await populated.afilter(
        query_time=datetime(2012, 1, 1, tzinfo=UTC), view="all"
    )
...
async def test_filter_uses_null_end_open_ended(populated):
    ...
    ids = await populated.afilter(
        query_time=datetime(2030, 1, 1, tzinfo=UTC), view="active"
    )
    assert len(ids) == 1
