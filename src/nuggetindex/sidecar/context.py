...
class ContextFormatter:
    ...
    def format(
        ...
        *,
        ...
        if contested:
            lines.append(
                "DISPUTED FACTS (sources disagree — surface the disagreement):"
            )
    ...
    def _format_fact(self, n: Nugget) -> str:
        start = (
            n.validity.start.date().isoformat() if n.validity else "?"
        )
        end = (
            n.validity.end.date().isoformat()
            if (n.validity and n.validity.end)
            else "present"
        )
    ...
    def _format_disputes(self, contested: list[Nugget]) -> list[str]:
        ...
        for (subj, pred), nuggs in groups.items():
            ...
            for n in nuggs:
                ...
                raw_evidence = (
                    n.provenance[0].evidence_span if n.provenance else ""
                )
                evidence = raw_evidence[:140]
                out.append(
                    f"    • {n.fact.object}  — {src}: \"{evidence}\""
                )
        return out
