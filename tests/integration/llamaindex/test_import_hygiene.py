"""Import-hygiene invariant: LlamaIndex adapter only uses public top-level API.

Same pattern as the LangChain integration's hygiene test: AST-parse each
``.py`` under ``src/nuggetindex/integrations/llamaindex`` and assert no
``ImportFrom`` hits any forbidden internal namespace. Letting the adapter
reach into ``nuggetindex.core.models`` or ``nuggetindex.store.backends``
...
def test_no_forbidden_internal_imports() -> None:
    ...
    assert not violations, (
        "integrations/llamaindex must only import from top-level nuggetindex.* "
